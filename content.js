...
(async () => {
    ...
    function handleLayoutChangeError() {
        ...
        zeptoQueryThirdPartyDOM('div.no')?.css({ opacity: 1, position: 'static' });
        alert(`Oops, looks like Gmail has updated its internal mechanism. We are currently looking into this. Sorry for the inconvenience.`);
        return false;
    }
    ...
    function sendTodo(todo, note = null) {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                zeptoQueryThirdPartyDOM('div.no')?.css({ opacity: 0, position: 'absolute', right: '-99999999px' });
                setTimeout(() => {
                    $composeButton.css({ pointerEvents: 'none' }); // Disable compose button while sending
                    const $composeEmailBottomPopup = zeptoQueryThirdPartyDOM('div.no:has(div[name="to"])');
                    const $toField = zeptoQueryThirdPartyDOM('div[name="to"].anm input');
                    const $subjectField = zeptoQueryThirdPartyDOM('input[name="subjectbox"][aria-label="Subject"]');
                    const $textboxField = zeptoQueryThirdPartyDOM('.AD [role="textbox"][aria-multiline="true"]');
                    ...
                    const $sendButton = zeptoQueryThirdPartyDOM('.AD [role="button"][aria-label^="Send"]');
